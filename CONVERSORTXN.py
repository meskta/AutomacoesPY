...
INPUT_DIR = r'N:\EXEMPLO\EXEMPLODIR'
OUTPUT_DIR = r'N:\EXEMPLO1\EXEMPLODIR\DIRINTERNO'
CONTROL_FILE = r'N:\EXEMPLO2\EXEMPLODIR\PLANILHA.XLSX'
...
def update_control_file(batch_number, successful_records):
    try:
        ...
        print(f"Planilha de controle atualizada com sucesso. Novo lote: {
              batch_number:06d}, Registros inseridos: {successful_records}")
    except Exception as e:
        ...
        with open(output_path, 'w') as f:
            ...
            for i, row in df.iterrows():
                try:
                    ...
                    detail_record = create_detail_record(str(card_number), str(
                        txn_code), value, date.strftime('%Y-%m-%d'), i + 1)
...
def main():
    try:
        ...
        output_filename = f"NIO_OFFLINE_{
            current_datetime.strftime('%d%m%Y_%H%M')}.txt"
        ...
        print(f"Total de registros processados com sucesso: {
              successful_records}")
        print(f"Total de registros na planilha original: {len(df)}")
        if successful_records < len(df):
            print(f"Atenção: {
                  len(df) - successful_records} registros não foram processados.")
